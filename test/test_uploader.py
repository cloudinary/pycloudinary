...
TEST_TRANS_SCALE2_PNG = dict(
    crop="scale", width="2.0", format="png", overlay=TEST_TRANS_OVERLAY_STR
)
...
class UploaderTest(unittest.TestCase):
    ...
    rbp_params = {
        ...
        "responsive_breakpoints": [
            {"create_derived": False, "transformation": {"angle": 90}, "format": "gif"},
            {
    ...
    def test_upload(self):
        ...
        result = uploader.upload(
            TEST_IMAGE, metadata=METADATA_FIELDS, tags=[UNIQUE_TAG]
        )
    ...
    def test_upload_path_lib_path(self):
        ...
        result = uploader.upload_large(
            path_lib_image_path, tags=[UNIQUE_TAG], resource_type="image"
        )
    ...
    def test_upload_custom_filename(self):
        ...
        result = uploader.upload(
            TEST_IMAGE, tags=[UNIQUE_TAG], filename=custom_filename
        )
        ...
        with io.BytesIO() as temp_file, open(TEST_IMAGE, "rb") as input_file:
            ...
            result = uploader.upload(
                temp_file, tags=[UNIQUE_TAG], filename=custom_filename
            )
    ...
    def test_upload_filename_override(self):
        ...
        result = uploader.upload(
            TEST_IMAGE, tags=[UNIQUE_TAG], filename_override="overridden"
        )
    ...
    def test_quality_analysis(self):
        ...
        result = uploader.upload(TEST_IMAGE, tags=[UNIQUE_TAG], quality_analysis=True)
        ...
        result = uploader.explicit(
            result["public_id"], type="upload", tags=[UNIQUE_TAG], quality_analysis=True
        )
    ...
    def test_upload_unicode_url(self):
        ...
        unicode_image_url = "{}".format(REMOTE_TEST_IMAGE)
...
""",
    ...
    def test_explicit(self):
        ...
        result_metadata = uploader.explicit(
            resource["public_id"],
            type="upload",
    ...
    def test_explicit_responsive_breakpoints_cache(self):
        ...
        explicit_result = uploader.explicit(
            upload_result["public_id"], **self.rbp_params
        )

        cache_value = cache.get(
            explicit_result["public_id"],
            transformation=self.rbp_trans,
            format=self.rbp_format,
        )
    ...
    def test_clear_invalid(self, mocker):
        ...
        uploader.update_metadata(
            METADATA_FIELDS, public_ids=[TEST_ID], clear_invalid=True
        )
    ...
    def test_upload_with_metadata(self):
        ...
        result = uploader.upload(
            TEST_IMAGE, tags=[UNIQUE_TAG], metadata=METADATA_FIELDS
        )
        self.assertEqual(
            METADATA_FIELD_VALUE, result["metadata"][METADATA_FIELD_UNIQUE_EXTERNAL_ID]
        )
    ...
    def test_explicit_with_metadata(self):
        ...
        resource = uploader.upload(TEST_IMAGE, tags=[UNIQUE_TAG])
        result = uploader.explicit(
            resource["public_id"], type="upload", metadata=METADATA_FIELDS
        )
        self.assertEqual(
            METADATA_FIELD_VALUE, result["metadata"][METADATA_FIELD_UNIQUE_EXTERNAL_ID]
        )
    ...
    def test_create_slideshow_from_manifest_transformation(self, mocker):
        ...
        self.assertEqual(
            "fn_render:" + slideshow_manifest,
            get_params(mocker)["manifest_transformation"],
        )
    ...
    def test_create_slideshow_from_manifest_json(self, mocker):
        ...
        slideshow_manifest_json_str = (
            '{"w":848,"h":480,"du":6,"fps":30,"vars":{"sdur":500,"tdur":500,'
        ...
        self.assertEqual(
            slideshow_manifest_json_str, get_params(mocker)["manifest_json"]
        )
    ...
    def test_tags(self):
        ...
        self.assertEqual(
            api.resource(result["public_id"])["tags"], ["tag1", "tag2", UNIQUE_TAG]
        )
        self.assertEqual(
            api.resource(result2["public_id"])["tags"], ["tag1", UNIQUE_TAG]
        )
        uploader.remove_tag("tag1", result["public_id"])
        self.assertEqual(
            api.resource(result["public_id"])["tags"], ["tag2", UNIQUE_TAG]
        )
    ...
    def test_multiple_tags(self):
        ...
        self.assertEqual(
            api.resource(result["public_id"])["tags"],
            ["tag1", "tag2", "tag3", "tag4", UNIQUE_TAG],
        )
        self.assertEqual(
            api.resource(result2["public_id"])["tags"],
            ["tag1", "tag2", "tag3", "tag4", UNIQUE_TAG],
        )
        ...
        self.assertEqual(
            api.resource(result["public_id"])["tags"], ["tag3", "tag4", UNIQUE_TAG]
        )
        self.assertEqual(
            api.resource(result2["public_id"])["tags"], ["tag1", "tag2", UNIQUE_TAG]
        )

        uploader.replace_tag(["tag5", UNIQUE_TAG], result["public_id"])
        uploader.replace_tag("tag7," + UNIQUE_TAG, result2["public_id"])
        self.assertEqual(
            api.resource(result["public_id"])["tags"], ["tag5", UNIQUE_TAG]
        )
        self.assertEqual(
            api.resource(result2["public_id"])["tags"], ["tag7", UNIQUE_TAG]
        )
    ...
    def test_allowed_formats_with_illegal_format(self):
        ...
        self.assertRaises(
            exceptions.Error, uploader.upload, TEST_IMAGE, allowed_formats=["jpg"]
        )
    ...
    def test_allowed_formats_with_format(self):
        ...
        result = uploader.upload(
            TEST_IMAGE, allowed_formats=["jpg"], format="jpg", tags=[UNIQUE_TAG]
        )
    ...
    def test_face_coordinates(self):
        ...
        result = uploader.upload(
            TEST_IMAGE, face_coordinates=coordinates, faces=True, tags=[UNIQUE_TAG]
        )
        ...
        self.assertEqual(
            {"faces": different_coordinates, "custom": [custom_coordinates]},
            info["coordinates"],
        )
    ...
    def test_detection(self):
        ...
        with six.assertRaisesRegex(
            self, exceptions.Error, "Detection invalid model 'illegal'"
        ):
    ...
    def test_upload_large_url(self):
        ...
        expected_signature = utils.api_sign_request(
            dict(public_id=resource4["public_id"], version=resource4["version"]),
            cloudinary.config().api_secret,
        )
    ...
    def test_responsive_breakpoints(self):
        result = uploader.upload(
            ...
            responsive_breakpoints=dict(
                create_derived=False, transformation=dict(angle=90)
            ),
        )

        self.assertIsNotNone(result["responsive_breakpoints"])
        self.assertEqual(result["responsive_breakpoints"][0]["transformation"], "a_90")
        ...
        result = uploader.explicit(
            ...
            tags=[UNIQUE_TAG],
            responsive_breakpoints=[
                dict(create_derived=False, transformation=dict(angle=90)),
                dict(create_derived=False, transformation=dict(angle=45)),
            ],
        )

        self.assertIsNotNone(result["responsive_breakpoints"])
        self.assertEqual(result["responsive_breakpoints"][0]["transformation"], "a_90")
        self.assertEqual(result["responsive_breakpoints"][1]["transformation"], "a_45")
    ...
    def test_access_control(self, request_mock):
        ...
        acl_2 = OrderedDict(
            (
                ...
                (
                    "start",
                    datetime.strptime("2019-02-22 16:20:57Z", "%Y-%m-%d %H:%M:%SZ"),
                ),
        ...
        expected_list_of_acl = (
            "[" + ",".join([v[1:-1] for v in (exp_acl, exp_acl_2, exp_acl_str)]) + "]"
        )
    ...
    def test_eval_upload_parameter(self):
        ...
        self.assertEqual(
            str(result["context"]["custom"]["width"]), str(TEST_IMAGE_WIDTH)
        )
    ...
    def test_generate_sprite(self):
        ...
        upload_result_1 = uploader.upload(
            TEST_IMAGE,
            tags=[sprite_test_tag, UNIQUE_TAG],
            public_id="sprite_test_tag_1{}".format(SUFFIX),
        )
        upload_result_2 = uploader.upload(
            TEST_IMAGE,
            tags=[sprite_test_tag, UNIQUE_TAG],
        ...
        result = uploader.generate_sprite(
            sprite_test_tag, transformation={"raw_transformation": "w_100"}
        )
    ...
    def test_multi(self):
        ...
        result = uploader.multi(
            tag=multi_test_tag, transformation={"raw_transformation": "c_crop,w_0.5"}
        )
