...
public_ids = [
    "{0}/search_test{1}_{1}".format(UNIQUE_TEST_FOLDER, i, SUFFIX)
    for i in range(0, TEST_IMAGES_COUNT)
]
upload_results = ["++"]
...
class SearchTest(unittest.TestCase):
    ...
    def test_should_add_sort_by_as_dict(self):
        ...
        self.assertEqual(
            query, {"sort_by": [{"created_at": "asc"}, {"updated_at": "desc"}]}
        )
    ...
    @unittest.skipUnless(cloudinary.config().api_secret, "requires api_key/api_secret")
    @unittest.skipIf(
        not os.environ.get("RUN_SEARCH_TESTS", False),
    ...
    def test_should_include_context(self):
        ...
        for res in results["resources"]:
            self.assertEqual([key for key in iterkeys(res["context"])], ["stage"])
    ...
    def test_should_not_duplicate_values(self, mocker):
        ...
        Search().sort_by("created_at", "asc").sort_by("public_id", "asc").sort_by(
            "created_at"
        ).aggregate("format").aggregate("format").aggregate("resource_type").with_field(
            "context"
        ).with_field(
            "context"
        ).with_field(
    ...
    def test_should_build_search_url(self):
        ...
        b64query = (
            "eyJleHByZXNzaW9uIjoicmVzb3VyY2VfdHlwZTppbWFnZSBBTkQgdGFncz1raXR0ZW4gQU5EIHVwbG9hZGVkX2F0"
        ...
        # default usage
        self.assertEqual(
            "https://res.cloudinary.com/{cloud}/search/{sig}/{ttl}/{query}".format(
                cloud=CLOUD_NAME, sig=ttl300_sig, ttl=300, query=b64query
            ),
            search.to_url(),
        )
        ...
        self.assertEqual(
            "https://res.cloudinary.com/{cloud}/search/{sig}/{ttl}/{query}/{cursor}".format(
                cloud=CLOUD_NAME,
                sig=ttl300_sig,
                ttl=300,
                query=b64query,
        ...
        # private cdn
        self.assertEqual(
            "https://{cloud}-res.cloudinary.com/search/{sig}/{ttl}/{query}".format(
                cloud=CLOUD_NAME, sig=ttl300_sig, ttl=300, query=b64query
            ),
    ...
    def test_should_search_folders_endpoint(self, mocker):
        ...
        SearchFolders().expression(FOLDERS_SEARCH_EXPRESSION).execute()
    ...
    def test_should_search_folders(self):
        results = SearchFolders().max_results(1).execute()
