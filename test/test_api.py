...
API_TEST_TRANS_SCALE100 = {
    "crop": "scale",
    "width": 100,
    "overlay": API_TEST_TRANS_OVERLAY_STR,
}
...
class ApiTest(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        cleanup_test_resources(
            [([API_TEST_ID, API_TEST_ID2, API_TEST_ID3, API_TEST_ID4, API_TEST_ID5],)]
        )
    ...
    def test03_resources_cursor(self):
        ...
        self.assertNotEqual(
            result2["resources"][0]["public_id"], result["resources"][0]["public_id"]
        )
    ...
    def test06_resources_tag(self, mocker):
        ...
        self.assertTrue(
            get_uri(mocker).endswith("/resources/image/tags/{}".format(API_TEST_TAG))
        )
        self.assertTrue(get_params(mocker)["context"])
        self.assertTrue(get_params(mocker)["tags"])
    ...
    def test06a_resources_by_ids(self, mocker):
        ...
        self.assertTrue(
            get_uri(mocker).endswith("/resources/image/upload"), get_uri(mocker)
        )
    ...
    def test06b_resources_by_asset_id(self, mocker):
        ...
        self.assertTrue(
            get_uri(mocker).endswith("/resources/by_asset_ids"), get_uri(mocker)
        )
    ...
    def test06c_resources_by_asset_ids(self, mocker):
        ...
        api.resources_by_asset_ids(
            [API_TEST_ASSET_ID, API_TEST_ASSET_ID2], context=True, tags=True
        )
    ...
    def test_visual_search(self, mocker):
        ...
        test_params = {
            "image_url": REMOTE_TEST_IMAGE,
            "image_asset_id": API_TEST_ASSET_ID,
            "text": "sample image",
        }
    ...
    def test07_a_resource_by_asset_id(self, mocker):
        ...
        self.assertTrue(
            get_uri(mocker).endswith("/resources/{}".format(API_TEST_ASSET_ID))
        )
    ...
    def test09b_delete_resources_by_tag(self, mocker):
        ...
        self.assertEqual(get_method(mocker), "DELETE")
        self.assertTrue(
            get_uri(mocker).endswith("/resources/image/tags/api_test_tag_for_delete")
        )
    ...
    def test09c_delete_resources_by_transformations(self, mocker):
        ...
        api.delete_resources(
            ["api_test", "api_test2"], transformations=["c_crop,w_100"]
        )
        self.assertEqual(get_method(mocker), "DELETE")
        ...
        self.assertEqual(get_method(mocker), "DELETE")
        self.assertEqual(
            get_param(mocker, "transformations"), "c_crop,w_100|c_scale,w_107"
        )
    ...
    def test_add_related_assets(self, mocker):
        ...
        self.assertTrue(
            get_uri(mocker).endswith(
                "/resources/related_assets/image/upload/" + API_TEST_ID
            )
        )
    ...
    def test_add_related_assets_by_asset_ids(self, mocker):
        ...
        api.add_related_assets_by_asset_ids(
            API_TEST_ASSET_ID, [API_TEST_ASSET_ID2, API_TEST_ASSET_ID3]
        )
        args, _ = mocker.call_args
        self.assertEqual(get_method(mocker), "POST")
        self.assertTrue(
            get_uri(mocker).endswith("/resources/related_assets/" + API_TEST_ASSET_ID)
        )
        param = get_json_body(mocker)["assets_to_relate"]
    ...
    def test_delete_related_assets_by_asset_ids(self, mocker):
        ...
        mocker.return_value = MOCK_RESPONSE
        api.delete_related_assets_by_asset_ids(
            API_TEST_ASSET_ID, [API_TEST_ASSET_ID2, API_TEST_ASSET_ID3]
        )
        args, _ = mocker.call_args
        self.assertEqual(get_method(mocker), "DELETE")
    ...
    def test12a_transformations_cursor(self, mocker):
        ...
        mocker.return_value = MOCK_RESPONSE
        api.transformation(
            API_TEST_TRANS_SCALE100, next_cursor=NEXT_CURSOR, max_results=10
        )
    ...
    def test14_transformation_update(self, mocker):
        ...
        self.assertEqual(
            API_TEST_TRANS_SCALE100_STR, get_params(mocker)["transformation"]
        )
    ...
    def test15a_transformation_unsafe_update(self):
        ...
        api.update_transformation(
            API_TEST_TRANS3, unsafe_update={"crop": "scale", "width": 103}
        )
    ...
    def test19_delete_derived(self):
        ...
        uploader.upload(
            TEST_IMAGE, public_id=API_TEST_ID5, eager=[{"width": 101, "crop": "scale"}]
        )
    ...
    def test20_manual_moderation(self):
        ...
        resource = uploader.upload(
            TEST_IMAGE, moderation="manual", tags=[UNIQUE_API_TAG]
        )
    ...
    def test22_raw_conversion(self):
        ...
        with six.assertRaisesRegex(self, BadRequest, "Illegal value"):
            api.update(
                resource["public_id"], raw_convert="illegal", resource_type="raw"
            )
    ...
    def test28_create_upload_preset(self, mocker):
        ...
        api.create_upload_preset(
            name=API_TEST_PRESET, folder="folder", live=True, eval=EVAL_STR
        )
    ...
    def test30_delete_upload_presets(self, mocker):
        ...
        self.assertEqual(get_method(mocker), "DELETE")
        self.assertTrue(
            get_uri(mocker).endswith("/upload_presets/{}".format(API_TEST_PRESET))
        )
    ...
    def test_folder_listing(self):
        ...
        uploader.upload(
            TEST_IMAGE, public_id="{}1/item".format(PREFIX), tags=[UNIQUE_API_TAG]
        )
        uploader.upload(
            TEST_IMAGE, public_id="{}2/item".format(PREFIX), tags=[UNIQUE_API_TAG]
        )
        uploader.upload(
            TEST_IMAGE,
        ...
        self.assertEqual(
            result["folders"][0]["path"], "{}1/test_subfolder1".format(PREFIX)
        )
        self.assertEqual(
            result["folders"][1]["path"], "{}1/test_subfolder2".format(PREFIX)
        )
    ...
    def test_restore(self):
        ...
        uploader.upload(
            TEST_IMAGE, public_id=RESTORE_TEST_ID, backup=True, tags=[UNIQUE_API_TAG]
        )
    ...
    def test_upload_mapping(self):
        api.create_upload_mapping(
            MAPPING_TEST_ID, template="http://cloudinary.com", tags=[UNIQUE_API_TAG]
        )
        ...
        self.assertIn(
            {"folder": MAPPING_TEST_ID, "template": "http://res.cloudinary.com"},
            result["mappings"],
        )
        api.delete_upload_mapping(MAPPING_TEST_ID)
        result = api.upload_mappings()
        self.assertNotIn(
            MAPPING_TEST_ID, [mapping.get("folder") for mapping in result["mappings"]]
        )
    ...
    def test_structured_metadata_in_resources_by_context_api(self):
        ...
        result = api.resources_by_context(
            API_TEST_CONTEXT_KEY, API_TEST_CONTEXT_VALUE1, metadata=True
        )
        ...
        result = api.resources_by_context(
            API_TEST_CONTEXT_KEY, API_TEST_CONTEXT_VALUE1, metadata=False
        )
