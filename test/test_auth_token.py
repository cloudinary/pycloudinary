...
@pytest.fixture
def config():
    url_backup = os.environ.get("CLOUDINARY_URL")
...
def test_generate_with_start_time_and_duration(config):
    token = cloudinary.utils.generate_auth_token(
        acl="/image/*",
        start_time=1111111111,
        duration=300,
        key=config.auth_token["key"],
    )
    assert token == (
        "__cld_token__=st=1111111111~exp=1111111411~acl=%2fimage%2f*~hmac"
        + "=1751370bcc6cfe9e03f30dd1a9722ba0f2cdca283fa3e6df3342a00a7528cc51"
    )
...
def test_should_add_token_for_public_resource():
    cloudinary.config(private_cdn=True)
    url, _ = cloudinary.utils.cloudinary_url(
        "sample.jpg",
        sign_url=True,
    ...
    assert (
        url
        == "http://test123-res.cloudinary.com/image/public/v1486020273/sample.jpg?__cld_token__=st"
        + "=11111111~exp=11111411~hmac="
        + "c2b77d9f81be6d89b5d0ebc67b671557e88a40bcf03dd4a6997ff4b994ceb80e"
    )


def test_should_not_add_token_if_signed_is_false():
    cloudinary.config(private_cdn=True)
    url, _ = cloudinary.utils.cloudinary_url(
...
def test_explicit_authToken_should_override_global_setting():
    cloudinary.config(private_cdn=True)
    url, _ = cloudinary.utils.cloudinary_url(
        "sample.jpg",
        sign_url=True,
...
def test_generate_token_string():
    ...
    token_options[
        "start_time"
    ] = 222222222  # we can't rely on the default "now" value in tests
...
def test_should_ignore_url_if_acl_is_provided():
    token_options = {
        "key": KEY,
        "duration": 300,
        "acl": "/image/*",
    ...
    assert acl_token == acl_token_url_ignored


def test_should_support_multiple_acls():
    token_options = {
        "key": KEY,
        "duration": 3600,
        "acl": ["/i/a/*", "/i/a/*", "/i/a/*"],
...
def test_must_provide_acl_or_url():
    with pytest.raises(Exception):
        cloudinary.utils.generate_auth_token(
            start_time=1111111111,
            duration=300,
...
def test_should_support_url_without_acl():
