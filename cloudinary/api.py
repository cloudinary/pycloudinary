...
def update(public_id, **options):
    ...
    upload_type = options.pop("type", "upload")
    uri = ["resources", resource_type, upload_type, public_id]
    params = only(options, "moderation_status", "raw_convert", "ocr",
...
def delete_derived_resources(derived_resource_ids, **options):
    ...
    params = [("derived_resource_ids[]", derived_resource_id)
              for derived_resource_id in derived_resource_ids]
    return call_api("delete", uri, params, **options)
...
def _call_api(method, uri, params=None, body=None, headers=None, **options):
    conf_prefix = cloudinary.config().upload_prefix
    prefix = options.pop("upload_prefix", conf_prefix) or "https://api.cloudinary.com"
