...
class ArchiveTest(unittest.TestCase):
    ...
    def test_download_folder(self):
        ...
        download_folder_url = utils.download_folder(
            folder_path="samples/", resource_type="image"
        )
        ...
        download_folder_url = utils.download_folder(
            folder_path="folder/", flatten_folders=True
        )
        ...
        download_folder_url = utils.download_folder(
            folder_path="folder/", expires_at=expiration_time
        )
        self.assertIn("expires_at", download_folder_url)

        # Should use original file_name of folder
        download_folder_url = utils.download_folder(
            folder_path="folder/", use_original_filename=True
        )
    ...
    def test_create_archive_multiple_resource_types(self, mocker):
        ...
        uploader.create_zip(resource_type="auto", fully_qualified_public_ids=test_ids)
    ...
    def test_download_backedup_asset(self):
        download_backedup_asset_url = utils.download_backedup_asset(
            "b71b23d9c89a81a254b88a91a9dad8cd", "0e493356d8a40b856c4863c026891a4e"
        )
