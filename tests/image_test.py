...
class ImageTest(unittest.TestCase):
    ...
    def test_build_url(self):
        """should generate url """
        self.assertEqual(
            self.image.build_url(),
            'http://res.cloudinary.com/test/image/upload/hello.png')
    ...
    def test_image(self):
        ...
        self.assertEqual(
            self.image.image(),
            '<img src="http://res.cloudinary.com/test/image/upload/hello.png"/>')
    ...
    def test_image_unicode(self):
        ...
        self.assertEqual(
            ...
            u'<img alt="\ua000abcd\u07b4" '
            u'src="http://res.cloudinary.com/test/image/upload/hello.png"/>')
    ...
    def test_scale(self):
        ...
        self.assertEqual(
            ...
            '<img height="100" '
            'src="http://res.cloudinary.com/test/image/upload/c_scale,h_100,w_100/hello.png" '
            'width="100"/>')
    ...
    def test_validate(self):
        ...
        self.assertFalse(
            CloudinaryImage("hello", format="png", version="1234",
                            signature="1234").validate())
        self.assertTrue(
            CloudinaryImage("hello", format="png", version="1234",
    ...
    def test_responsive_width(self):
        ...
        self.assertEqual(
            ...
            'data-src="http://res.cloudinary.com/test/image/upload/c_limit,w_auto/hello.png"/>')
    ...
    def test_dpr_auto(self):
        ...
        self.assertEqual(
            ...
            '<img class="cld-hidpi" '
            'data-src="http://res.cloudinary.com/test/image/upload/dpr_auto/hello.png"/>')
    ...
    def shared_client_hints(self, **options):
        ...
        six.assertRegex(
            self, tag,
            'src=["\']http://res.cloudinary.com/test/image/'
            'upload/c_scale,dpr_auto,w_auto/sample.jpg["\']',
            "should not use data-src or set responsive class")
        cloudinary.config(responsive=True)
        tag = CloudinaryImage('sample.jpg').image(**options)
        six.assertRegex(self, tag, '<img.*>')
        self.sixAssertNotRegex(tag, '<.* class.*>', "should override responsive")
        self.sixAssertNotRegex(tag, '\bdata-src\b', "should override responsive")
        six.assertRegex(
            self, tag,
            'src=["\']http://res.cloudinary.com/test/image/'
            'upload/c_scale,dpr_auto,w_auto/sample.jpg["\']',
    ...
    def test_client_hints_as_false(self):
        ...
        cloudinary.config(responsive=True)
        tag = CloudinaryImage('sample.jpg').image(
            width="auto", crop="scale", cloud_name="test", client_hints=False)
        six.assertRegex(self, tag, '<img.*>')
        six.assertRegex(self, tag, 'class=["\']cld-responsive["\']')
        six.assertRegex(
            self, tag,
            'data-src=["\']http://res.cloudinary.com/test/image/'
            'upload/c_scale,w_auto/sample.jpg["\']')

    def test_width_auto_breakpoints(self):
        """supports auto width"""
        tag = CloudinaryImage('sample.jpg').image(
            crop="scale", dpr="auto", cloud_name="test",
            width="auto:breakpoints", client_hints=True)
        six.assertRegex(
            self, tag,
            'src=["\']http://res.cloudinary.com/test/image/'
            'upload/c_scale,dpr_auto,w_auto:breakpoints/sample.jpg["\']')
