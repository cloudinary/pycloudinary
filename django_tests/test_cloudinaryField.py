...
from django_tests.helper_test import (
    SUFFIX,
    TEST_IMAGE,
    TEST_IMAGE_W,
    TEST_IMAGE_H,
...
class TestCloudinaryField(TestCase):
    ...
    def setUpTestData(cls):
        Poll.objects.create(
            question="with image", image="image/upload/v1234/{}.jpg".format(API_TEST_ID)
        )
    ...
    def test_to_python(self):
        ...
        self.assertEqual(
            c.to_python("{}.jpg".format(API_TEST_ID)).build_url(), res.build_url()
        )
    ...
    def test_upload_options_with_filename(self):
        ...
        with mock.patch(
            "cloudinary.uploader.upload_resource", return_value=mocked_resource
        ) as upload_mock:
    ...
    def test_pre_save(self):
        c = CloudinaryField(
            "image", width_field="image_width", height_field="image_height"
        )
        c.set_attributes_from_name("image")
        mocked_resource = cloudinary.CloudinaryResource(
            metadata={"width": TEST_IMAGE_W, "height": TEST_IMAGE_H},
        ...
        with mock.patch(
            "cloudinary.uploader.upload_resource", return_value=mocked_resource_empty
        ) as upload_mock:
            c.pre_save(self.p, None)
    ...
    def test_get_prep_value(self):
        ...
        self.assertEqual(
            c.get_prep_value(res), "image/upload/{}.jpg".format(API_TEST_ID)
        )
    ...
    def test_value_to_string(self):
        ...
        self.assertEqual(
            "image/upload/v1234/{name}.jpg".format(name=API_TEST_ID), value_string
        )
    ...
    def test_image_field(self):
        ...
        self.assertEqual(
            ...
            "/{cloud}/image/upload/v1234/{name}.jpg".format(
                cloud=cloudinary.config().cloud_name, name=API_TEST_ID
            ),
        )
