...
class ImageTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.full_public_id = "{id}.{format}".format(
            id=self.public_id, format=self.image_format
        )
        ...
        self.fill_transformation_str = "c_fill,h_{h},w_{w}".format(
            h=self.max_width, w=self.max_width
        )
    ...
    def test_build_url(self):
        """should generate url"""
        self.assertEqual(
            self.image.build_url(), "{url}/{id}".format(**self.common_format)
        )
    ...
    def test_image(self):
        ...
        self.assertEqual(
            self.image.image(), '<img src="{url}/{id}"/>'.format(**self.common_format)
        )

    def test_image_unicode(self):
        """should generate image with unicode arguments"""
        self.assertEqual(
            self.image.image(alt="\ua000abcd\u07b4"),
            '<img alt="\ua000abcd\u07b4" src="{url}/{id}"/>'.format(
                **self.common_format
            ),
        )
    ...
    def test_scale(self):
        ...
        self.assertEqual(
            ...
            '<img height="100" src="{url}/c_scale,h_100,w_100/{id}" width="100"/>'.format(
                **self.common_format
            ),
        )
    ...
    def test_validate(self):
        ...
        self.assertFalse(
            CloudinaryImage(
                "hello", format="png", version="1234", signature="1234"
            ).validate()
        )
        self.assertTrue(
            CloudinaryImage(
                ...
                format="png",
                version="1234",
                signature="2aa73bf69fb50816e5509e32275b8c417dcb880d",
            ).validate()
        )
    ...
    def test_responsive_width(self):
        ...
        self.assertEqual(
            self.image.image(responsive_width=True),
    ...
    def test_width_auto(self):
        ...
        self.assertEqual(
            self.image.image(width="auto", crop="limit"),
            '<img class="cld-responsive" data-src="{url}/c_limit,w_auto/{id}"/>'.format(
                **self.common_format
            ),
        )
    ...
    def test_dpr_auto(self):
        ...
        self.assertEqual(
            self.image.image(dpr="auto"),
            '<img class="cld-hidpi" data-src="{url}/dpr_auto/{id}"/>'.format(
                **self.common_format
            ),
        )
    ...
    def test_effect_art_incognito(self):
        ...
        e = "art:incognito"
        self.assertEqual(
            self.image.image(effect=e),
            '<img src="{url}/e_{e}/{id}"/>'.format(e=e, **self.common_format),
        )
    ...
    def shared_client_hints(self, **options):
        ...
        six.assertRegex(
            self, tag, "<img.*>", "should not use data-src or set responsive class"
        )
        self.assertIsNone(
            re.match("<.* class.*>", tag),
            "should not use data-src or set responsive class",
        ...
        self.assertIsNone(
            re.match("\bdata-src\b", tag),
            "should not use data-src or set responsive class",
        )
        expected_re = "src=[\"']{url}/c_scale,dpr_auto,w_auto/{id}[\"']".format(
            **self.common_format
        )
        six.assertRegex(
            self, tag, expected_re, "should not use data-src or set responsive class"
        )
    ...
    def test_client_hints_as_global(self):
        ...
        self.shared_client_hints(
            dpr="auto", cloud_name=self.cloud_name, width="auto", crop="scale"
        )

    def test_client_hints_as_false(self):
        """should use normal responsive behaviour"""
        cloudinary.config(responsive=True)
        tag = CloudinaryImage(self.full_public_id).image(
            width="auto", crop="scale", cloud_name=self.cloud_name, client_hints=False
        )
        six.assertRegex(self, tag, "<img.*>")
        six.assertRegex(self, tag, "class=[\"']cld-responsive[\"']")
        exp = "data-src=[\"']{url}/c_scale,w_auto/{id}[\"']".format(
            **self.common_format
        )
    ...
    def test_width_auto_breakpoints(self):
        ...
        expected_re = (
            "src=[\"']{url}/c_scale,dpr_auto,w_auto:breakpoints/{id}[\"']".format(
                **self.common_format
            )
        )
        six.assertRegex(self, tag, expected_re)

    def _common_image_tag_helper(
        self,
        tag_name,
        public_id,
        common_trans_str,
        custom_trans_str=None,
        srcset_breakpoints=None,
        attributes=None,
        is_void=False,
    ):
        """
        ...
        if srcset_breakpoints:
            ...
            attributes["srcset"] = ", ".join(
                bp_template.format(w=bp) for bp in srcset_breakpoints
            )

        attributes_str = (
            " ".join(
                '{k}="{v}"'.format(k=k, v=attributes[k]) for k in sorted(attributes)
            )
            if attributes
            else ""
        )
        ...
        tag += (
            "/>" if not is_void else ">"
        )  # HTML5 void elements do not need to be self closed

        self.logger.debug(re.sub(r'([,"]) ', r"\1\n    ", tag))

        return tag

    def _get_expected_cl_image_tag(
        self,
        public_id,
        common_trans_str,
        custom_trans_str=None,
        ...
        return self._common_image_tag_helper(
            "img",
            public_id,
            common_trans_str,
            custom_trans_str,
    ...
    def _get_expected_cl_source_tag(
        self,
        public_id,
        common_trans_str,
        custom_trans_str=None,
    ...
    def test_srcset_from_breakpoints(self):
        ...
        tag = CloudinaryImage(self.full_public_id).image(
            srcset=self.common_srcset, **self.common_image_options
        )
        expected_tag = self._get_expected_cl_image_tag(
            self.full_public_id,
    ...
    def test_srcset_from_min_width_max_width_max_images(self):
        ...
        tag = CloudinaryImage(self.full_public_id).image(
            srcset=srcset_params, **self.common_image_options
        )
    ...
    def test_srcset_with_one_image(self):
        ...
        tag_by_params = CloudinaryImage(self.full_public_id).image(
            srcset=srcset_params, **self.common_image_options
        )
        ...
        tag_by_breakpoint = CloudinaryImage(self.full_public_id).image(
            srcset=srcset_breakpoint, **self.common_image_options
        )
    ...
    def test_srcset_with_sizes_attribute(self):
        ...
        expected_sizes_attr = ", ".join(
            "(max-width: {w}px) {w}px".format(w=bp) for bp in self.breakpoint_list
        )
    ...
    def test_srcset_from_string(self):
        ...
        tag = CloudinaryImage(self.full_public_id).image(
            attributes=attributes, **self.common_image_options
        )

        expected_tag = self._get_expected_cl_image_tag(
            self.full_public_id, self.common_transformation_str, attributes=attributes
        )
        self.assertEqual(expected_tag, tag)

        legacy_tag = CloudinaryImage(self.full_public_id).image(
            srcset=raw_srcset_value, **self.common_image_options
        )
    ...
    def test_srcset_invalid_values(self):
        ...
        with mock.patch("cloudinary.logger") as log_mock:
            for invalid_srcset in invalid_srcset_params:
                image_tag = CloudinaryImage(self.full_public_id).image(
                    srcset=invalid_srcset, **self.common_image_options
                )
                self.assertNotIn("srcset", image_tag)

            expected_log_call_count = (
                len(invalid_srcset_params) + 1
            )  # When `sizes` is True we call log twice
    ...
    def test_custom_attributes(self):
        ...
        tag = CloudinaryImage(self.full_public_id).image(
            attributes=self.custom_attributes, **self.common_image_options
        )
    ...
    def test_source_tag(self):
        ...
        expected_tag = self._get_expected_cl_source_tag(
            self.full_public_id, self.common_transformation_str
        )
    ...
    def test_source_tag_media_query(self):
        ...
        expected_media = "(min-width: {min}px) and (max-width: {max}px)".format(
            min=self.min_width, max=self.max_width
        )
        expected_tag = self._get_expected_cl_source_tag(
            self.full_public_id, "", attributes={"media": expected_media}
        )
    ...
    def test_source_tag_responsive_srcset(self):
        ...
        expected_tag = self._get_expected_cl_source_tag(
            self.full_public_id, "", srcset_breakpoints=self.breakpoint_list
        )
    ...
    def test_picture_tag(self):
        ...
        tag = CloudinaryImage(self.full_public_id).picture(
            sources=[
                {
        ...
        expected_source_1 = self._get_expected_cl_source_tag(
            self.full_public_id,
            "{tr}/a_17,e_sepia,w_{w}".format(
                tr=self.fill_transformation_str, w=self.min_width
            ),
        ...
        expected_source_2 = self._get_expected_cl_source_tag(
            self.full_public_id,
            "{tr}/a_18,e_colorize,w_{w}".format(
                tr=self.fill_transformation_str, w=self.max_width
            ),
            media={"min_width": self.min_width, "max_width": self.max_width},
        )
