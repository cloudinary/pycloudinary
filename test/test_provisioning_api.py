...
class AccountApiTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if (
            ...
            or not config.provisioning_api_key
            or not config.provisioning_api_secret
        ):
            return
    ...
    @unittest.skipUnless(
        cloudinary.provisioning.account_config().account_id, "requires account_id"
    )
    ...
    @unittest.skipUnless(
        cloudinary.provisioning.account_config().provisioning_api_secret,
    ...
    def test_get_all_sub_accounts(self):
        ...
        sub_account_by_id = [
            sub_account
            for sub_account in res["sub_accounts"]
    ...
    def test_update_user(self):
        ...
        res = cloudinary.provisioning.update_user(
            self.user_id_1, new_name, new_email_address
        )
        ...
        user_by_id = [user for user in res["users"] if user["id"] == self.user_id_1]
    ...
    def test_get_users_by_prefix(self):
        res_1 = cloudinary.provisioning.users(
            pending=True, prefix=self.user_name_2[:-1]
        )
        res_2 = cloudinary.provisioning.users(
            pending=True, prefix=self.user_name_2 + "zzz"
        )
    ...
    def test_get_users_by_sub_account_id(self):
        res = cloudinary.provisioning.users(
            pending=True, user_ids=[self.user_id_2], sub_account_id=self.cloud_id
        )
        self.assertEqual(len(res["users"]), 1)
    ...
    def test_get_users_by_nonexistent_sub_account_id(self):
        with six.assertRaisesRegex(
            self,
            NotFound,
            "Cannot find sub account with id {}".format(UNIQUE_SUB_ACCOUNT_ID),
        ):
            cloudinary.provisioning.users(
                pending=True, sub_account_id=UNIQUE_SUB_ACCOUNT_ID
            )
    ...
    def test_add_remove_user_from_group(self):
        ...
        remove_users_from_group_resp = cloudinary.provisioning.remove_user_from_group(
            self.group_id, self.user_id_1
        )
    ...
    def test_get_user_groups(self):
        ...
        group_by_id = [
            user_group
            for user_group in res["user_groups"]
    ...
    def test_generate_access_key(self):
        ...
        res = cloudinary.provisioning.generate_access_key(
            self.cloud_id, name=key_name, enabled=False
        )
    ...
    def test_update_access_key(self):
        ...
        key_res = cloudinary.provisioning.generate_access_key(
            self.cloud_id, name=key_name, enabled=True
        )

        self.assertEqual(key_name, key_res["name"])
        self.assertEqual(True, key_res["enabled"])

        res = cloudinary.provisioning.update_access_key(
            self.cloud_id, key_res["api_key"], name=updated_key_name, enabled=False
        )
